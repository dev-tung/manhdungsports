    ...
    <main class="Main">
        <div class="MainContent">
            ...
            <div class="ListSearchTotal">
                <span class="ListSearchTotalItem">{{ $customer->sum('product_quantity') }} khách hàng</span>
            </div>

            <div class="List">
                @foreach( $customer as $customer )
                    <a class="ListItem" href="{{route('product.edit', $customer->product_id)}}">
                        <div class="ListItemthumbnail">
                            @if( !empty( $customer->product_thumbnail ) )
                                <img class="ListItemthumbnail" src="{{ asset($customer->product_thumbnail) }}">
                            @else
                        ...
                        <div class="ListItemInfo">
                            <h4 class="ListItemName">{{ $customer->product_name }}</h4>
                            <div class="ListSpanGroup">
                                <span class="ListItemSpan">Tồn kho {{ $customer->product_quantity }}</span>
                                <span>-</span>
                                <span class="ListItemSpan">Giá nhập {{ number_format($customer->product_price_input) }} đ</span>
                                <span>-</span>
                                <span class="ListItemSpan">Giá bán {{ number_format($customer->product_price_output) }} đ</span>
                            </div>
                        </div>
                    </a>
                @endforeach
